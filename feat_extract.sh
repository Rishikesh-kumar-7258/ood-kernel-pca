...
CUDA_VISIBLE_DEVICES=0 python /kaggle/working/ood-kernel-pca/feat_extract_largescale.py \
 --arch R50 --in_data ImageNet --out_data ${out_data} --batch_size 128

CUDA_VISIBLE_DEVICES=0 python /kaggle/working/ood-kernel-pca/feat_extract_largescale.py \
 --arch R50 --in_data ImageNet --out_data ${out_data} --batch_size 128 \
 --model_path /kaggle/input/supcon/pytorch/default/1/supcon.pth --supcon
